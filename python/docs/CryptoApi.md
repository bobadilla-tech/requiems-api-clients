...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    api_instance = openapi_client.CryptoApi(api_client)
    symbol = 'BTC' # str | Uppercase ticker symbol (e.g. BTC, ETH, SOL)

    try:
        # Get Crypto Price
        api_response = api_instance.v1_finance_crypto_symbol_get(symbol)
        print("The response of CryptoApi->v1_finance_crypto_symbol_get:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling CryptoApi->v1_finance_crypto_symbol_get: %s\n" % e)
```
...
------------- | ------------- | ------------- | -------------
 **symbol** | **str**| Uppercase ticker symbol (e.g. BTC, ETH, SOL) | 
