...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    api_instance = openapi_client.DomainInfoApi(api_client)
    domain = 'example.com' # str | The domain to look up (e.g. example.com)

    try:
        # Get Domain Info
        api_response = api_instance.v1_networking_domain_domain_get(domain)
        print("The response of DomainInfoApi->v1_networking_domain_domain_get:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling DomainInfoApi->v1_networking_domain_domain_get: %s\n" % e)
```
